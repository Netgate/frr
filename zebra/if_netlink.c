...
#define AF_BRIDGE 7
	...
	if (h->nlmsg_type == RTM_NEWLINK) {
		...
		if (tb[IFLA_IFALIAS]) {
			...
			/*
			 * If this is a linux-cp tap associated with a loopback,
			 * an alias like 'loop[0-9]+' should be set.
			 * Adjust the flags so it looks more like frr expects
			 * a loopback to look.
			 */
#define LB_PFX "loop"
#define LB_PFX_LEN (sizeof(LB_PFX) - 1)

			const char *alias = RTA_DATA(tb[IFLA_IFALIAS]);
			if ((strlen(alias) > LB_PFX_LEN) &&
			    !strncmp(alias, LB_PFX, LB_PFX_LEN) &&
			    isdigit(alias[LB_PFX_LEN])) {
